...
-- @Last Modified time: 2018-10-22 21:02:26
...
function LuaUIHelper.bind( gObj , panel)
	...
	local behaviour = gObj:GetComponentInParent(typeof(LuaPageBehaviour))
    panel.behaviour = behaviour

    for _,widget in pairs(panel.widgets) do
        local childTrans = trans
        if not string.isEmptyOrNil(widget.path) then
            local widgetPath = string.gsub(widget.path , "%." , "/")
            childTrans = trans:Find(widgetPath)
        end
        ...
        if childTrans then
            ...
            Debugger.Log("Cant find Child ! Path is " .. widget.path)
        end
    end
end
...
function LuaUIHelper.setPanelDepthOffset( root , offsetDepth )
    local panels = root:GetComponentsInChildren(typeof(UnityEngine.Canvas))
...
function LuaUIHelper.reflushDepth(gObj , panel)
    local panels = gObj:GetComponentsInChildren(typeof(UnityEngine.Canvas) , true)
...
--- 释放界面组件绑定
function LuaUIHelper.unbind( panel)

    if not panel.widgets then   return   end
    ...
    for _,widget in pairs(panel.widgets) do
        ...
        local luaCom = widget.src
        if widget.src == LuaScript then
            LuaUIHelper.unbind(widget)
        end

        panel[widget.field] = nil
    end
end
