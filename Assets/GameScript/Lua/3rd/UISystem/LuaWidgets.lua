...
-- @Last Modified time: 2018-06-28 19:17:39
...
--------------基础 UGUI Canvas---------------
LuaCanvas = class("LuaCanvas")
implement(LuaCanvas , LuaBaseWidget)

function LuaCanvas:bind( trans , widget )

    local sprite = trans.gameObject:GetComponentInParent(typeof(UnityEngine.Canvas))
...
LuaTexture = class("LuaTexture")
implement(LuaTexture , LuaBaseWidget)

function LuaTexture:bind( trans , widget )

    local sprite = trans.gameObject:GetComponent(typeof(UnityEngine.UI.RawImage))
    tolua.setpeer(sprite , self)
...
function LuaInput:bind( trans , widget ,behaviour)
    ...
    if widget.onChange then
        behaviour:AddInputFieldChanged(trans.gameObject , widget.onChange)
    end

    if widget.onSubmit then
        behaviour:AddInputFieldSubmit(trans.gameObject , widget.onSubmit)
    end
...
function LuaSlider:bind( trans , widget ,behaviour)
    ...
    if widget.onValueChange then
        behaviour:AddSliderValueChange(trans.gameObject , widget.onValueChange)
    end
...
function LuaScrollBar:bind( trans , widget ,behaviour)
    ...
    if widget.onValueChange then
        behaviour:AddScrollderbarValueChange(trans.gameObject , widget.onValueChange)
    end
