...
namespace GEX.Utility
{
	public class Crypto
	{
        ...
        public static byte[] SimpleCrypto(byte[] buf, int secret = 0xffff)
        {
        ...
        public static byte[] SimpleCrypto(string path)
        {
            if (File.Exists(path))
                return SimpleCrypto(File.ReadAllBytes(path));
            else
